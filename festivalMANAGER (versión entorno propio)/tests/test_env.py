...
# 5. Custom spaces test: Verifies our MultiDiscreteSpace and BoxSpace behave correctly.
def test_custom_spaces():
    env = make_env()

    # --- MultiDiscreteSpace ---
    # sample() returns a valid action
    for _ in range(20):
        action = env.action_space.sample()
        assert env.action_space.contains(action), \
            f"sample() produced invalid action: {action}"

    # contains() rejects wrong shapes
    assert not env.action_space.contains(np.array([0, 0])), \
        "contains() should reject wrong-shape array"

    # contains() rejects out-of-range values
    bad = np.array([999, 999, 999, 999], dtype=np.int64)
    assert not env.action_space.contains(bad), \
        "contains() should reject out-of-range values"

    # --- BoxSpace ---
    for _ in range(10):
        obs_sample = env.observation_space.sample()
        assert obs_sample.shape == env.observation_space.shape, \
            f"BoxSpace sample wrong shape: {obs_sample.shape}"
        assert obs_sample.min() >= 0.0 and obs_sample.max() <= 1.0, \
            "BoxSpace sample out of [0, 1]"

    # contains() accepts a valid observation
    obs, _ = env.reset()
    assert env.observation_space.contains(obs), \
        "observation_space.contains() rejected a real observation"

    # contains() rejects wrong shape
    assert not env.observation_space.contains(np.zeros(3)), \
        "BoxSpace.contains() should reject wrong-shape array" 
