...
# Instead of using all 47 variables (which would generate ~10^47 unique states),
# we select only the 4 most relevant ones so the agent can revisit states and learn.
def _discretize(obs):
    ...
    # Discretize each continuous value [0,1] into one of 5 buckets (0 to 4)
    # For example: happiness=0.72 → int(0.72*5)=3
    h = int(np.clip(happiness * 5, 0, 4))
    r = int(np.clip(revenue   * 5, 0, 4))
    t = int(np.clip(time_slot * 5, 0, 4))
    q = int(np.clip(avg_queue * 5, 0, 4))

    # Combine into a single unique integer using base 5
...
def _run_episode(agent, env, greedy=False):
    ...
    while True:
        ...
        # Convert flat index to MultiDiscrete action expected by FestivalEnv
        ...
        raw_next, reward, terminated, truncated, _ = env.step(action_for_env)
...
def train_mode(mode):
    ...
    # Determine n_actions from our custom MultiDiscreteSpace

    if isinstance(env.action_space, MultiDiscreteSpace):
        n_actions = int(np.prod(env.action_space.nvec))
    else:
        print(f"ERROR: tipo de action_space no reconocido: {type(env.action_space)}")
    ...
    for ep in range(1, t_cfg['n_episodes'] + 1):
        ...
        agent.decay_epsilon()
        rewards_log.append((ep, round(ep_reward, 4), round(agent.epsilon, 6)))
 
        if ep % t_cfg['log_interval'] == 0:
            # Moving average over the last log_interval episodes
    ...
    eval_rewards = [_run_episode(agent, env, greedy=True) for _ in range(t_cfg['eval_episodes'])]
